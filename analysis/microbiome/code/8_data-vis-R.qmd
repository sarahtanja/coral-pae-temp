...
metadata <- readr::read_tsv("../output/mesocosm/sample-metadata-mesocosm.tsv")
metadata <- metadata %>% rename('SampleID' = '#SampleID')
ASV <- read_qza("../output/mesocosm/table-mesocosm.qza")$data
taxonomy <- read_qza("../output/taxonomy/classification.qza")$data %>% parse_taxonomy()

taxasums <- summarize_taxa(ASV, taxonomy)$Genus

taxa_barplot(taxasums, metadata, "PaeTemp")

```

### [How to create a relative abundance barplot using ggplot](https://github.com/surh/scip_barplot/blob/master/extended_example.md) with [video](https://www.youtube.com/watch?v=siIoupAnILk) by Sur Herrera Paredes

```{r}
metadata <- readr::read_tsv("../output/mesocosm/sample-metadata-mesocosm.tsv")
metadata <- metadata %>% rename('sample_id' = '#SampleID')

ASV <- read_qza("../output/mesocosm/table-mesocosm.qza")$data 

ASVtib <- as_tibble(ASV, rownames = "feature_id")
ASVtib
```
...
```{r}
feature_table <- ASVlong %>% 
  left_join(metadata, by = "sample_id")

taxonomy <- read_qza("../output/taxonomy/classification.qza")$data %>% parse_taxonomy()
taxonomytib <- as_tibble(taxonomy, rownames = "feature_id")

feature_table_taxa <- feature_table %>% 
  left_join(taxonomytib, by = "feature_id")
```
...
feature_table %>%
  ...
  geom_bar(aes(fill = feature_id), stat = "identity", position = "fill")

```


```{r}
p1 <- feature_table_taxa %>%
  ggplot(aes(x = sample_id, y = count)) +
  facet_grid(~ PaeTemp + Colony, scales = "free_x", space = "free_x") +
